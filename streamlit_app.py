"""
streamlit_app.py — Verdict Watch V14
Full model selector: choose any Gemini or Groq model.
Primary = Gemini (Google AI) | Fallback = Groq
"""
...
.test-ico{{font-size:1.1rem;flex-shrink:0;}}.test-tag{{font-size:.78rem;font-weight:700;color:var(--t1);flex:1;}}.test-type{{font-size:.68rem;color:var(--t3);}}
.test-badge{{font-size:.68rem;font-weight:700;border-radius:var(--r-pill);padding:2px 8px;}}
.test-pending{{background:var(--surf3);color:var(--t3);}}.test-pass{{background:var(--grn-lt);color:var(--green);}}.test-fail{{background:var(--red-lt);color:var(--red);}}
...
/* Model selector card */
.model-card{{background:var(--surf2);border:1.5px solid var(--border);border-radius:var(--r-lg);padding:.75rem 1rem;margin-bottom:6px;cursor:pointer;transition:var(--trans);}}
.model-card:hover{{border-color:var(--accent);background:var(--acc-lt);}}
.model-card.selected{{border-color:var(--accent);background:var(--acc-lt);}}
.model-name{{font-size:.8rem;font-weight:700;color:var(--t1);}}
.model-desc{{font-size:.7rem;color:var(--t3);margin-top:2px;}}
.provider-badge{{display:inline-block;border-radius:var(--r-pill);padding:2px 8px;font-size:.65rem;font-weight:700;margin-bottom:4px;}}
.provider-gemini{{background:rgba(26,35,126,.25);color:#8AB4F8;border:1px solid #8AB4F8;}}
.provider-groq{{background:rgba(251,176,64,.12);color:#FBB040;border:1px solid #FBB040;}}
...
EXAMPLES = [
    ...
    {"tag":"Graduate School Rejection (Race Proxy)","type":"university","expect_bias":True,
    ...
    {"tag":"Insurance Claim Denial (Socioeconomic)","type":"other","expect_bias":True,
...
_DEFS = {
    ...
    "decision_input":"","dtype_sel":"job","scan_mode":"full",
...
def ring_svg(pct, bias, size=110):
    r = size*.38; cx=cy=size/2; sw=size*.09
    ...
    return (
        ...
        f'<text x="{cx}" y="{cy-size*.04}" text-anchor="middle" font-family="JetBrains Mono,monospace" font-size="{size*.18}" font-weight="600" fill="{col}">{pct}%</text>'
        f'<text x="{cx}" y="{cy+size*.12}" text-anchor="middle" font-family="Syne,sans-serif" font-size="{size*.07}" font-weight="700" fill="{tok("--t3")}" letter-spacing="0.08em">CONF</text>'
        f'</svg>'
    )
...
def txt_report(report, text, dtype):
    tm=report.get("timing_ms",{}); laws=report.get("legal_frameworks",[]); recs=report.get("recommendations",[])
    prov=report.get("ai_provider","gemini"); model=report.get("ai_model",current_model())
    lines=["="*64,"       VERDICT WATCH V14 — BIAS ANALYSIS REPORT","="*64,
           f"Generated  : {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}",
           f"Type       : {dtype.upper()}",f"Report ID  : {report.get('id','N/A')}",
...
def to_csv(reps):
    rows=[{
        "id":r.get("id",""),"created_at":(r.get("created_at") or "")[:16].replace("T"," "),
        "mode":r.get("mode","full"),"ai_provider":r.get("ai_provider","gemini"),
        "bias_found":r.get("bias_found",False),"severity":r.get("severity",""),
        "confidence":int(r.get("confidence_score",0)*100),
        "bias_types":"; ".join(r.get("bias_types",[])),
        "affected":r.get("affected_characteristic",""),
        "original":r.get("original_outcome",""),"fair":r.get("fair_outcome",""),
...
def chart_pie(b,c):
    fig=go.Figure(go.Pie(labels=["Bias Detected","No Bias"],values=[max(b,1),max(c,1)],hole=.68,
        marker={"colors":[tok("--red"),tok("--green")],"line":{"color":tok("--bg"),"width":3}},
        textfont={"family":"Syne,sans-serif","size":11},textinfo="percent",
        hovertemplate="%{label}: %{value}<extra></extra>"))
    total=b+c or 1
    fig.add_annotation(text=f"<b style='font-size:20px'>{total}</b><br><span style='font-size:9px;color:{tok('--t3')}'>TOTAL</span>",
        x=.5,y=.5,showarrow=False,font={"family":"JetBrains Mono,monospace","size":18,"color":tok("--t1")})
...
def chart_bar(items,max_n=8):
    ...
    ll=list(labels)
    fig=go.Figure(go.Bar(x=list(values),y=ll,orientation="h",
    ...
    fig.update_layout(height=max(150,len(ll)*38+40),
        xaxis={"showgrid":True,"gridcolor":tok("--surf3"),"zeroline":False,"tickfont":{"family":"JetBrains Mono,monospace","size":9}},
        yaxis={"tickfont":{"family":"Syne,sans-serif","size":9}},bargap=.4,
        margin=dict(l=10,r=30,t=10,b=10),**_base())
    return fig

def chart_sparkline(scores):
    if not scores: scores=[0]
    fig=go.Figure(go.Scatter(y=scores,mode="lines",line={"color":tok("--accent"),"width":2},
        fill="tozeroy",fillcolor="rgba(107,138,255,0.10)",hovertemplate="Score %{y}%<extra></extra>"))
    fig.update_layout(height=75,xaxis={"visible":False},yaxis={"range":[0,105],"visible":False},
        margin={"l":0,"r":0,"t":4,"b":0},paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
        font={"family":"Syne,sans-serif"})
    return fig

def chart_trend(td):
    if not td: return None
    dates=[d.get("date","") for d in td]; rates=[d.get("bias_rate",0) for d in td]; totals=[d.get("total",0) for d in td]
    fig=go.Figure()
    ...
    fig.add_trace(go.Scatter(x=dates,y=rates,name="Bias %",mode="lines+markers",
        line={"color":tok("--red"),"width":2.5},marker={"color":tok("--red"),"size":5,"line":{"color":tok("--bg"),"width":1.5}},
...
def chart_radar(all_r):
    ...
    fig=go.Figure(go.Scatterpolar(r=vals+[vals[0]],theta=BIAS_DIMS+[BIAS_DIMS[0]],fill="toself",
        fillcolor="rgba(107,138,255,0.10)",line={"color":tok("--accent"),"width":2},marker={"color":tok("--accent"),"size":5}))
    fig.update_layout(polar={"bgcolor":"rgba(0,0,0,0)",
        ...
        height=250,showlegend=False,margin={"l":40,"r":40,"t":20,"b":20},
        paper_bgcolor="rgba(0,0,0,0)",font={"family":"Syne,sans-serif"})
    return fig

def chart_gauge(val,bias):
    col=tok("--red") if bias else tok("--green")
    fig=go.Figure(go.Indicator(mode="gauge+number",value=round(val*100),
        ...
        gauge={"axis":{"range":[0,100],"tickwidth":0,"tickfont":{"color":tok("--t3"),"size":8}},
               ...
               "steps":[{"range":[0,33],"color":"rgba(22,101,52,.07)"},{"range":[33,66],"color":"rgba(146,64,14,.07)"},{"range":[66,100],"color":"rgba(196,43,43,.07)"}]}))
...
def run_analysis(text, dtype, mode="full", provider="gemini", model=None):
    # Temporarily override the model in services if specified
...
def render_result(report, dt, dtype, compact=False):
    bias=report.get("bias_found",False); conf=report.get("confidence_score",0.0); pct=int(conf*100)
    btype=report.get("bias_types",[]); aff=report.get("affected_characteristic","")
    orig=report.get("original_outcome","N/A"); fair=report.get("fair_outcome","N/A")
    expl=report.get("explanation",""); recs=report.get("recommendations",[])
    laws=report.get("legal_frameworks",[]); evid=report.get("bias_evidence","")
    tm=report.get("timing_ms",{}); mode_=report.get("mode","full")
    prov=report.get("ai_provider","gemini"); mdl=report.get("ai_model","")
    ...
    model_badge = f'<span style="background:var(--surf3);color:var(--t2);border:1px solid var(--border);border-radius:999px;padding:2px 9px;font-size:.67rem;font-weight:600;">{mdl_short}</span>' if mdl_short else ""
    ...
    with rc1:
        aff_html=""
        if aff:
            aff_html=(f'<div style="margin-top:9px;"><div class="card-lbl">Affected</div>'
    ...
    if evid:
        st.markdown(f'<div class="card card-warn"><div class="card-lbl">Bias Evidence</div><div class="card-val" style="font-size:.83rem;">{evid}</div></div>',unsafe_allow_html=True)
    if tm:
    ...
    if not compact:
        if dt and (btype or report.get("bias_phrases")):
            ...
            st.markdown(f'<div class="hl-box">{hl}</div><div style="font-size:.66rem;color:{tok("--t3")};margin-top:3px;">Highlighted = potential bias proxies</div>',unsafe_allow_html=True)
        if expl:
...
with st.sidebar:
    gem_status = gemini_ok(); groq_status = groq_ok()
    dot_cls = "api-ok" if gem_status else ("api-warn" if groq_status else "api-err")
    status_txt = ("Gemini + Groq ready" if gem_status and groq_status else
    ...
    mdl_short = _trunc(cur_model.replace("gemini-","G·").replace("llama-","L·").replace("-versatile","").replace("-instant",""),18)
    ...
    st.markdown(
        ...
        f'{"🔵" if cur_prov=="gemini" else "🟠"} {mdl_short}</div>'
        f'</div>',
        unsafe_allow_html=True)
...
if view == "models":
    ...
    with prov_col1:
        ...
        st.markdown(
            f'<div class="card" style="{gem_border}cursor:pointer;">'
            ...
            f'{"<div style=\"font-size:.65rem;color:var(--accent);margin-top:5px;font-weight:700;\">✓ SELECTED</div>" if gem_sel else ""}'
            f'</div>',
            unsafe_allow_html=True)
    ...
    with prov_col2:
        ...
        st.markdown(
            f'<div class="card" style="{grq_border}cursor:pointer;">'
            f'<div style="font-size:1.3rem;margin-bottom:6px;">🟠</div>'
            ...
            f'{"<div style=\"font-size:.65rem;color:var(--amber);margin-top:5px;font-weight:700;\">✓ SELECTED</div>" if grq_sel else ""}'
            f'</div>',
            unsafe_allow_html=True)
    ...
    if current_prov == "gemini":
        ...
        for mid, mdesc in GEMINI_MODELS.items():
            ...
            bg_style = f"background:var(--acc-lt);" if is_sel else ""
            rec_badge = ' <span style="background:rgba(74,222,128,.15);color:#4ADE80;border-radius:999px;padding:1px 7px;font-size:.6rem;font-weight:700;border:1px solid #4ADE80;">✦ Recommended</span>' if recommended else ""
            ...
            with col_a:
                st.markdown(
                    ...
                    f'</div>'
                    f'{"<div style=\"font-size:.72rem;color:var(--accent);font-weight:700;\">✓ Active</div>" if is_sel else ""}'
                    f'</div></div>',
                    unsafe_allow_html=True)
            with col_b:
                if not is_sel:
        ...
        for mid, mdesc in GROQ_MODELS.items():
            ...
            bg_style = f"background:var(--amb-lt);" if is_sel else ""
            rec_badge = ' <span style="background:rgba(251,176,64,.15);color:#FBB040;border-radius:999px;padding:1px 7px;font-size:.6rem;font-weight:700;border:1px solid #FBB040;">✦ Recommended</span>' if recommended else ""
            ...
            with col_a:
                st.markdown(
                    ...
                    f'<div style="font-size:.82rem;font-weight:700;color:var(--t1);">{mid}{rec_badge}</div>'
                    f'<div style="font-size:.72rem;color:var(--t3);margin-top:2px;">{mdesc}</div>'
                    f'{"<div style=\"font-size:.72rem;color:var(--amber);font-weight:700;margin-top:3px;\">✓ Active</div>" if is_sel else ""}'
                    f'</div>',
                    unsafe_allow_html=True)
            with col_b:
                if not is_sel:
    ...
    st.markdown("<br>",unsafe_allow_html=True)
    st.markdown('<div class="card card-info"><div class="card-lbl">Currently Active</div>'
        ...
        f'<div style="font-size:.7rem;color:var(--t3);margin-top:4px;">{"Gemini auto-falls back to Groq if API call fails" if current_prov=="gemini" else "Groq direct — no fallback"}</div>'
        f'</div>',unsafe_allow_html=True)

    if st.button("⚡ Go to Analyse →", key="goto_analyse_from_models"):
        st.session_state["view"] = "analyse"; st.rerun()
...
# ─────────────────────────────────────────────────────
    ...
    banner_color = tok("--acc-lt") if cur_prov=="gemini" else tok("--amb-lt")
    banner_border = tok("--accent") if cur_prov=="gemini" else tok("--amber")

    st.markdown(
        f'<div style="background:{banner_color};border:1px solid {banner_border};border-radius:var(--r-lg);'
        f'padding:.55rem 1rem;margin-bottom:.9rem;display:flex;align-items:center;justify-content:space-between;">'
        f'<div style="display:flex;align-items:center;gap:8px;">'
        f'<span style="font-size:.75rem;font-weight:700;color:var(--t1);">Active Model:</span>'
        f'{provider_badge_html(cur_prov)}'
        f'<span style="font-family:var(--ff-m);font-size:.72rem;color:var(--t2);">{cur_model}</span>'
        f'{"" if prov_available else " <span style=\"color:var(--red);font-size:.7rem;\">⚠ Key missing</span>"}'
        f'</div>'
        f'<a href="#" style="font-size:.7rem;color:var(--accent);font-weight:600;text-decoration:none;" '
        f'onclick="window.location.reload()">Change model →</a>'
        f'</div>',
        unsafe_allow_html=True)

    if not prov_available:
        fallback_prov = "groq" if cur_prov=="gemini" else "gemini"
        fallback_ok = groq_status if cur_prov=="gemini" else gem_status
        if fallback_ok:
            st.markdown(f'<div class="card card-warn"><div class="card-val" style="font-size:.8rem;">⚠ {cur_prov.title()} key missing — will auto-fallback to {fallback_prov.title()}</div></div>',unsafe_allow_html=True)
    ...
    with input_col:
        ...
        if "Paste" in mode_sel:
            decision_text = st.text_area("text",label_visibility="collapsed",height=190,
                key="decision_input",
            ...
            if uf:
                ...
                if ex:
                    decision_text=ex
                    st.markdown(f'<div style="margin-bottom:7px;"><span class="chip cg">✓ {len(ex):,} chars from {uf.name}</span></div>',unsafe_allow_html=True)
                    with st.expander("Preview"):
                        st.text(_trunc(ex,600))

        opts=["job","loan","medical","university","other"]
        cur=st.session_state.get("dtype_sel","job"); idx=opts.index(cur) if cur in opts else 0
        ...
        scan_mode=st.radio("Scan Mode",["full","quick"],
    ...
    with right_pad:
        ...
        # Quick model switch in right panel
        st.markdown(
            f'<div class="card" style="margin-top:2px;border-left:3px solid var(--accent);">'
            f'<div class="card-lbl">Quick Switch Model</div>'
        ...
        for p, models in quick_models.items():
            ...
            for m in models:
                ...
                bg_    = f"background:{'var(--acc-lt)' if p=='gemini' else 'var(--amb-lt)'};" if is_cur else ""
                border_= f"border-color:{'var(--accent)' if p=='gemini' else 'var(--amber)'};" if is_cur else ""
                if st.button(f"{'✓ ' if is_cur else ''}{short}",key=f"qs_{p}_{m}",use_container_width=True):
        ...
        st.markdown(
            ...
            f'<div style="margin-top:6px;">'+"".join(f'<span class="chip cn" style="margin-bottom:3px;">{d}</span>' for d in BIAS_DIMS)+
            f'</div></div>',
            unsafe_allow_html=True)
    ...
    if run_btn:
        ...
        if not dt:
            ...
            th=services.hash_text(dt); cached=services.find_duplicate(th)
            if cached and not st.session_state.get("force_rerun"):
                st.markdown('<div class="dup-warn">⚠ <div><strong>Identical text — showing cached result.</strong><br>Click Re-run for a fresh analysis.</div></div>',unsafe_allow_html=True)
            ...
            if compare_mode and not err:
                ...
                if dt2:
                    with st.spinner("Analysing Decision B…"):
                        rb,eb=run_analysis(dt2,st.session_state.get("dtype_b","other"),
                                           mode=scan_mode,
                                           provider=st.session_state.get("ai_provider","gemini"),
                                           model=st.session_state.get("ai_model","gemini-1.5-flash"))
    ...
    if compare_mode and st.session_state.get("cmp_ra") and st.session_state.get("cmp_rb"):
        ...
        if ba and bb: msg=f"Both show bias — Decision {'A' if ca>=cb_ else 'B'} has higher confidence ({int(max(ca,cb_)*100)}%)"
        ...
        for col,r,lbl in [(v1,ra,"A"),(v2,rb,"B")]:
            with col:
                b_=r.get("bias_found",False); vcls_="vb-bias" if b_ else "vb-clean"; vt_="⚠ Bias" if b_ else "✓ Clean"
                st.markdown(f'<div class="vb {vcls_}" style="padding:1rem;"><div class="vb-title" style="font-size:1.1rem;">Decision {lbl}</div><div class="vb-sub" style="font-size:.78rem;">{vt_}</div></div>',unsafe_allow_html=True)
                ...
                prov_r=r.get("ai_provider","gemini"); mdl_r=r.get("ai_model","")
                st.markdown(f'<div style="margin-top:4px;">{sev_badge(r.get("confidence_score",0),b_,r.get("severity","low"))} {provider_badge_html(prov_r)}</div>',unsafe_allow_html=True)
                if r.get("fair_outcome"):
                    st.markdown(f'<div class="card card-ok" style="margin-top:7px;"><div class="card-lbl">Fair Outcome</div><div class="card-val serif">{r["fair_outcome"]}</div></div>',unsafe_allow_html=True)
        st.stop()

    if not report:
        st.markdown('<div class="empty"><div class="empty-ico">⚖</div><div class="empty-t">No analysis yet</div><div class="empty-s">Paste a decision above and click Run Analysis.</div></div>',unsafe_allow_html=True)
    else:
        ...
        with fb1:
            if st.button("👍 Helpful",key="fb_y"): services.save_feedback(report.get("id",""),1,fb_comment); st.success("Thanks!")
        with fb2:
            if st.button("👎 Not helpful",key="fb_n"): services.save_feedback(report.get("id",""),0,fb_comment); st.info("Noted.")
        ...
        with dl1:
            st.download_button("↓ Full Report (.txt)",data=txt_report(report,dt,dtype_),
                file_name=f"verdict_v14_{(report.get('id') or 'r')[:8]}.txt",mime="text/plain",key="dl_rpt")
        with dl2:
            st.download_button("↓ CSV",data=to_csv([report]),
                file_name=f"verdict_v14_{(report.get('id') or 'r')[:8]}.csv",mime="text/csv",key="dl_csv_single")
...
# ─────────────────────────────────────────────────────
    ...
    hist=all_reports()
    if not hist:
        st.markdown('<div class="empty"><div class="empty-ico">◎</div><div class="empty-t">Nothing to show yet</div><div class="empty-s">Run your first analysis to populate the dashboard.</div></div>',unsafe_allow_html=True)
    else:
        b_reps=[r for r in hist if r.get("bias_found")]; c_reps=[r for r in hist if not r.get("bias_found")]
        ...
        avg_sev="High" if sum(sev_vals)/len(sev_vals)>=2.5 else ("Medium" if sum(sev_vals)/len(sev_vals)>=1.5 else "Low")
        ...
        with dl1:
            st.download_button("↓ Export CSV",data=to_csv(hist),
                file_name=f"verdict_dash_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv",key="dash_dl")
...
# ─────────────────────────────────────────────────────
    ...
    if not hist:
        ...
        if q:
            ql=q.lower()
            filt=[r for r in filt if ql in (r.get("affected_characteristic") or "").lower()
        ...
        with hdr1: st.markdown(f'<div style="font-size:.73rem;color:{tok("--t3")};margin-bottom:9px;">Showing {len(filt)} of {len(hist)} reports</div>',unsafe_allow_html=True)
        with hdr2: st.download_button("↓ CSV",data=to_csv(filt),file_name=f"verdict_hist_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv",key="hist_dl")

        for r in filt:
            bias=r.get("bias_found",False); conf=int(r.get("confidence_score",0)*100)
            aff=r.get("affected_characteristic") or "—"; created=(r.get("created_at") or "")[:16].replace("T"," ")
            ico="⚠" if bias else "✓"; mode_lbl="Quick" if r.get("mode")=="quick" else "Full"
            prov_r=r.get("ai_provider","gemini"); prov_ico="🔵" if prov_r=="gemini" else "🟠"
            with st.expander(f'{ico} {"Bias" if bias else "No Bias"} · {conf}% · {aff} · {created} [{prov_ico}{mode_lbl}]'):
                ec1,ec2=st.columns(2,gap="large")
                with ec1:
                    vcls="card-err" if bias else "card-ok"; vt="⚠ Bias Detected" if bias else "✓ No Bias Found"
                    st.markdown(
                ...
                st.download_button("↓ Report (.txt)",data=txt_report(r,"","other"),
                    file_name=f"verdict_{(r.get('id') or 'x')[:8]}.txt",mime="text/plain",key=f"dl_{r.get('id','x')}")
...
# ─────────────────────────────────────────────────────
    ...
    st.markdown(f'<div class="ps">Analyse up to 10 decisions at once. Separate with <code>---</code> or upload CSV with a <code>text</code> column.</div>',unsafe_allow_html=True)

    if not any_api_ok():
        st.markdown('<div class="key-err">⚠ API key missing — see Settings.</div>',unsafe_allow_html=True)

    # Active model display
    cur_prov=st.session_state.get("ai_provider","gemini"); cur_model=st.session_state.get("ai_model","gemini-1.5-flash")
    st.markdown(f'<div class="card card-info" style="margin-bottom:.8rem;"><div class="card-lbl">Active Model for Batch</div><div style="margin-top:4px;">{provider_badge_html(cur_prov)} <span style="font-family:var(--ff-m);font-size:.78rem;">{cur_model}</span></div></div>',unsafe_allow_html=True)
    ...
    if "Paste" in bmode:
        ...
        if bf:
            try:
                dfu=pd.read_csv(bf)
                if "text" in dfu.columns: blocks=dfu["text"].dropna().tolist(); st.markdown(f'<span class="chip cg">✓ {len(blocks)} rows loaded</span>',unsafe_allow_html=True)
                else: st.error("CSV must have a 'text' column.")
            except Exception as e: st.error(f"❌ {e}")

    bc1,bc2,bc3=st.columns([2,1,1])
    with bc1: btype=st.selectbox("Decision type",["job","loan","medical","university","other"],format_func=lambda x:TYPE_LABELS[x],label_visibility="collapsed",key="b_type")
    with bc2: scan_b=st.radio("Mode",["full","quick"],horizontal=True,format_func=lambda x:"Full" if x=="full" else "Quick",key="b_scan")
    with bc3: brun=st.button("⊞ Run Batch",key="b_run",disabled=not any_api_ok())

    if blocks: st.markdown(f'<span class="chip cb">● {len(blocks)} decision{"s" if len(blocks)!=1 else ""} queued</span>',unsafe_allow_html=True)
    ...
    if brun:
        if not blocks: st.warning("⚠ No decisions found.")
            ...
            for i,blk in enumerate(blocks):
                ...
                status.markdown(f'<div style="font-size:.78rem;color:{tok("--accent")};font-weight:600;">Analysing {i+1}/{len(blocks)}{"  · ETA ~"+str(int(eta))+"s" if eta>1 else ""}…</div>',unsafe_allow_html=True)
            ...
            m1.metric("Total",len(results)); m2.metric("Bias Detected",b_c); m3.metric("No Bias",c_c); m4.metric("Errors",e_c)
            ...
            for i,res in enumerate(results,1):
                ...
                if err: rows.append({"#":i,"Verdict":"ERROR","Conf":"—","Bias Types":str(err)[:50],"Provider":"—"})
                    ...
                    rows.append({"#":i,"Verdict":"⚠ Bias" if rep.get("bias_found") else "✓ Clean",
                                 "Conf":f"{int(rep.get('confidence_score',0)*100)}%",
                                 "Bias Types":", ".join(rep.get("bias_types",[])) or "None",
                                 "Provider":f"{'🔵' if rep.get('ai_provider','gemini')=='gemini' else '🟠'} {rep.get('ai_model','')}"})
            if rows: st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)

            all_r=[r["report"] for r in results if isinstance(r.get("report"),dict)]
            if all_r:
                dl1,_=st.columns([1,3])
                with dl1: st.download_button("↓ CSV",data=to_csv(all_r),file_name=f"verdict_batch_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",mime="text/csv",key="b_dl")
...
# ─────────────────────────────────────────────────────
    ...
    cur_prov=st.session_state.get("ai_provider","gemini"); cur_model=st.session_state.get("ai_model","gemini-1.5-flash")
    st.markdown(f'<div class="card card-info" style="margin-bottom:.8rem;"><div class="card-lbl">Model for Tests</div><div style="margin-top:4px;">{provider_badge_html(cur_prov)} <span style="font-family:var(--ff-m);font-size:.78rem;">{cur_model}</span></div></div>',unsafe_allow_html=True)
    ...
    if not any_api_ok():
        ...
        for i,ex in enumerate(EXAMPLES,1):
            ...
            st.markdown(f'<div class="test-row"><div class="test-ico">{ico}</div><div style="flex:1;"><div class="test-tag">{i}. {ex["tag"]}</div><div class="test-type">{TYPE_LABELS[ex["type"]]} · {"Bias expected" if ex["expect_bias"] else "Clean expected"}</div></div><span class="test-badge test-pending">Pending</span></div>',unsafe_allow_html=True)

        st.markdown("<br>",unsafe_allow_html=True)
        ts_mode=st.radio("Scan mode",["quick","full"],format_func=lambda x:"Quick (faster)" if x=="quick" else "Full (detailed)",horizontal=True,key="ts_mode")
        ...
        if run_all:
            ...
            for i,ex in enumerate(EXAMPLES):
                ...
                ph_s=st.empty(); ph_s.markdown(f'<div style="font-size:.78rem;color:{tok("--accent")};font-weight:600;">Running {i+1}/{len(EXAMPLES)}: {ex["tag"]}…</div>',unsafe_allow_html=True)
                ...
                if err:
                    ...
                    passed=(got==ex["expect_bias"]); status="pass" if passed else "fail"
            ...
            acc=tsc["pass"]/len(EXAMPLES); acc_col=tok("--green") if acc>=.8 else (tok("--amber") if acc>=.5 else tok("--red"))
            st.markdown(f'<div class="char-track" style="height:5px;margin:10px 0 18px;"><div class="char-fill" style="width:{int(acc*100)}%;background:{acc_col};height:100%;"></div></div>',unsafe_allow_html=True)

            for i,res in enumerate(ts_results,1):
                ex=res["ex"]; status=res["status"]
                ...
                with st.expander(f'{ico} Test {i}: {ex["tag"]}  [{badge_lbl}]',expanded=(status!="pass")):
                    ...
                    if res["err"]: st.error(f"Error: {res['err']}")
                    else:
                        rep=res["rep"]; got=rep.get("bias_found",False); exp=ex["expect_bias"]; conf_=int(rep.get("confidence_score",0)*100)
                        ecol,gcol=st.columns(2,gap="small")
                        with ecol:
                            exp_cls="card-warn" if exp else "card-muted"
                            st.markdown(f'<div class="card {exp_cls}"><div class="card-lbl">Expected</div><div class="card-val mono">{"⚠ Bias" if exp else "✓ No Bias"}</div></div>',unsafe_allow_html=True)
                        with gcol:
                            got_cls="card-err" if got else "card-ok"; match_lbl="✓ Match" if (got==exp) else "✗ Mismatch"
                            st.markdown(f'<div class="card {got_cls}"><div class="card-lbl">Got · {match_lbl}</div><div class="card-val mono">{"⚠ Bias" if got else "✓ No Bias"} ({conf_}%)</div></div>',unsafe_allow_html=True)
                        if rep.get("bias_types"): st.markdown(f'<div class="card card-muted" style="margin-top:6px;"><div class="card-lbl">Bias Types</div><div>{chips(rep.get("bias_types",[]))}</div></div>',unsafe_allow_html=True)
            ...
            st.markdown('<hr class="div">',unsafe_allow_html=True)
            test_rows=[{"test_n":i,"tag":res["ex"]["tag"],"type":res["ex"]["type"],
            ...
            with dl1:
                st.download_button("↓ Test Report (CSV)",data=pd.DataFrame(test_rows).to_csv(index=False),
                    file_name=f"verdict_tests_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",mime="text/csv",key="ts_dl")
...
# ─────────────────────────────────────────────────────
            ...
            if st.button("⊛ Test Groq Connection",key="test_groq"):
                with st.spinner("Testing Groq…"):
                    try:
                        ...
                        c.chat.completions.create(model=services._GROQ_MODEL,max_tokens=5,messages=[{"role":"user","content":"ping"}])
    ...
    with sc2:
        ...
        cur_prov=st.session_state.get("ai_provider","gemini"); cur_model=st.session_state.get("ai_model","gemini-1.5-flash")
        all_r=all_reports(); fb=services.get_feedback_stats(); db_url=os.getenv("DATABASE_URL","sqlite:///verdict_watch.db")
        st.markdown(
            f'<div class="card card-info"><div class="card-lbl">Active Provider</div>'
            f'<div style="margin-top:5px;">{provider_badge_html(cur_prov)} <span style="font-family:var(--ff-m);font-size:.78rem;">{cur_model}</span></div></div>'
            f'<div class="card" style="margin-top:6px;"><div class="card-lbl">Total Reports</div><div class="card-val mono lg">{len(all_r)}</div></div>'
            f'<div class="card" style="margin-top:6px;"><div class="card-lbl">Database</div><div class="card-val mono" style="font-size:.73rem;">{db_url}</div></div>'
            f'<div class="card card-info" style="margin-top:6px;"><div class="card-lbl">User Feedback</div><div class="card-val mono">{fb["total"]} ratings · {fb["helpful_pct"]}% helpful</div></div>',
            unsafe_allow_html=True)
        ...
        for ico,name,desc in [
            ...
            st.markdown(f'<div class="feat-row"><span><span class="feat-ico">{ico}</span><span class="feat-name">{name}</span></span><span class="feat-desc">{desc}</span></div>',unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────
    ...
    with ab1:
        st.markdown(
            f'<div class="card" style="background:var(--surf2);margin-bottom:12px;">'
            f'<div style="font-family:var(--ff-d);font-size:1.1rem;color:var(--t1);margin-bottom:6px;">What is Verdict Watch?</div>'
            f'<div style="font-size:.82rem;color:var(--t2);line-height:1.75;">A 3-step pipeline using Google Gemini (primary) with Groq fallback — extracting decision criteria, detecting discriminatory patterns across 7 bias dimensions, citing relevant laws, and generating the fair outcome you deserved.</div></div>',
            unsafe_allow_html=True)

        st.markdown('<div class="lbl">AI Models Available</div>',unsafe_allow_html=True)
        gem_col, grq_col = st.columns(2, gap="small")
        with gem_col:
            st.markdown('<div class="card card-info"><div class="card-lbl">🔵 Gemini Models</div><div style="margin-top:5px;">'+"".join(f'<div style="font-size:.72rem;color:var(--t1);padding:2px 0;font-family:var(--ff-m);">{m}</div>' for m in GEMINI_MODELS)+"</div></div>",unsafe_allow_html=True)
        with grq_col:
            st.markdown('<div class="card card-warn"><div class="card-lbl">🟠 Groq Models</div><div style="margin-top:5px;">'+"".join(f'<div style="font-size:.72rem;color:var(--t1);padding:2px 0;font-family:var(--ff-m);">{m}</div>' for m in GROQ_MODELS)+"</div></div>",unsafe_allow_html=True)

        st.markdown('<div class="lbl" style="margin-top:10px;">Bias Dimensions Detected</div>',unsafe_allow_html=True)
        for name,desc in [("Gender Bias","Gender, name, or parental status"),("Age Discrimination","Unfair weighting of age group"),("Racial / Ethnic Bias","Name-based or origin profiling"),("Geographic Redlining","Zip code as discriminatory proxy"),("Socioeconomic Bias","Employment sector over-weighting"),("Language Discrimination","Primary language used against applicants"),("Insurance Classification","Insurance tier ranking treatment")]:
            st.markdown(f'<div class="card" style="margin-bottom:5px;"><div class="card-lbl">{name}</div><div class="card-val" style="font-size:.82rem;">{desc}</div></div>',unsafe_allow_html=True)

    with ab2:
        st.markdown('<div class="lbl">Tech Stack</div>',unsafe_allow_html=True)
        for name,desc in [("Google Gemini","Primary AI (google-generativeai)"),("Groq / Llama","Fallback AI"),("FastAPI","REST API"),("Streamlit ≥ 1.35","Web UI"),("SQLAlchemy","Database ORM"),("SQLite","Zero-config storage"),("Plotly","Interactive charts"),("DM Serif Display","Heading font"),("Syne","UI font"),("JetBrains Mono","Data font")]:
            st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--surf3);"><span style="font-size:.82rem;font-weight:500;color:var(--t1);">{name}</span><span style="font-size:.73rem;color:var(--t3);">{desc}</span></div>',unsafe_allow_html=True)
        st.markdown("<br>",unsafe_allow_html=True)
        st.markdown('<div class="card card-warn"><div class="card-lbl">⚠ Disclaimer</div><div class="card-val" style="font-size:.79rem;">Not legal advice. Built for educational awareness only. Consult a qualified legal professional for discrimination claims.</div></div>',unsafe_allow_html=True)
        st.markdown(f'<div style="text-align:center;font-size:.68rem;color:var(--t3);margin-top:12px;">Verdict Watch V14 · Gemini PRIMARY + Groq FALLBACK</div>',unsafe_allow_html=True)